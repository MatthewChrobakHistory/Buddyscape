...
Sub ProcessNpcMovement(ByVal MapNpcNum As Long)
    ...
    If MapNpc(MapNpcNum).Moving = MOVING_WALKING Then
        ...
        Select Case MapNpc(MapNpcNum).Dir
            Case DIR_UP
                MapNpc(MapNpcNum).YOffset = MapNpc(MapNpcNum).YOffset - ((ElapsedTime / 1000) * (Npc(MapNpcNum).Speed * SIZE_X))
            ...
            Case DIR_LEFT
                MapNpc(MapNpcNum).XOffset = MapNpc(MapNpcNum).XOffset - ((ElapsedTime / 1000) * (Npc(MapNpcNum).Speed * SIZE_X))
