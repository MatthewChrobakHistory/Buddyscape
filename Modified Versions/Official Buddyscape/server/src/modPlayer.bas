...
' ToDo
Sub OnDeath(ByVal index As Long, Optional ByVal Killer As Long = 0)
    Dim i As Long, x As Long
    Dim Max As Byte
    Dim GItem(1 To 5) As Long
    Dim Temp As Long
    Dim EP As Long
    ...
    If Killer <> 0 Then
        For i = 1 To MAX_INV
            If Player(Killer).Inv(i).Num = 256 Then
                EP = Player(Killer).Inv(i).Value
                Player(Killer).Inv(i).Value = 0
                Player(Killer).Inv(i).Num = 0
                Exit For
            End If
        Next
        Call GiveInvItem(Killer, 256, RAND(1, 4))
        For x = 1 To Equipment.Equipment_Count - 1
            If Player(index).Equipment(x) > 0 Then
                Call GiveInvItem(Killer, 256, RAND(1, 2))
            End If
            If Player(Killer).Equipment(x) > 0 Then
                Call GiveInvItem(Killer, 256, RAND(1, 2))
            End If
        Next
    End If
    
    ' Drop untradable items
    For i = 1 To MAX_INV
        If Player(index).Inv(i).Num > 0 Then
            If Item(Player(index).Inv(i).Num).price = 11 Then
                Player(index).Inv(i).Num = 0
                Player(index).Inv(i).Value = 0
            Else
                Call PlayerMapDropItem(index, i, Player(index).Inv(i).Value)
            End If
        End If
    Next
    ...
    For i = 1 To MAX_INV
        If Player(index).Inv(i).Num > 0 Then
            If Item(Player(index).Inv(i).Num).Rarity = 1 Then
                Player(index).Inv(i).Num = 0
                Player(index).Inv(i).Value = 0
            Else
                Call PlayerMapDropItem(index, i, Player(index).Inv(i).Value)
    ...
    If EP > 0 And Player(index).Map = 88 Then
        EP = EP * 100000 ' 100k
        Max = 3 + RAND(0, 3)
        For i = 1 To Max
            ...
            Do While Found = False
                Temp = RAND(1, MAX_ITEMS)
                If Item(Temp).price > 10 And Item(Temp).price <= EP Then
                    Call GiveInvItem(index, Temp, 1)
                    Found = True
                End If
            Loop
        Next
    ...
    If Player(index).Map = 88 Then
        For i = 1 To MAX_MAP_ITEMS
            If MapItem(88, i).Num > 0 Then
                MapItem(88, i).playerName = vbNullString
                MapItem(88, i).playerTimer = 0
            ' send updates to everyone
            End If
        Next
        SendMapItemsToAll 88
    End If
        ...
        Select Case Item(itemnum).Type
            ...
            Case ITEM_TYPE_CONSUME
                ...
                If itemnum = 255 Then
                    ...
                    Do While i = 0
                        i = RAND(1, MAX_ITEMS)
                        If Item(i).price > 10 Then
                            Call GiveInvItem(index, i, 1, True)
                        Else
                            i = 0
                        End If
                    Loop
                ...
                If itemnum = 254 Then
                    For i = 1 To MAX_INV
                        If Player(index).Inv(i).Num = 0 Then
                            Call GiveInvItem(index, 8, 1, True)
                        End If
                    Next
                End If
