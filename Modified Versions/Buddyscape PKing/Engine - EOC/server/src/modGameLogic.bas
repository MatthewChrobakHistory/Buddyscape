...
'if the task we're on is bigger than the number of tasks (finished the last task) ie on task 51 out of 50
If Player(index).PlayerQuest(QuestNum).TaskOn > Quest(QuestNum).TaskCount Then
    'make the status "Finished"
    Player(index).PlayerQuest(QuestNum).QuestStatus = 2
    Call SendPlayerData(index)
    Call PlayerMsg(index, "Congrats! You have finished the quest " & Trim$(Quest(QuestNum).Name) & "!", BrightRed)
End If
...
If Player(index).PlayerQuest(QuestIndex).QuestStatus = 0 Then
    ...
    Call SendPlayerData(index)
    Call PlayerMsg(index, "You started the quest " & Trim$(Quest(QuestIndex).Name), Cyan)
Else
    Call PlayerMsg(index, "You have already started this quest!", BrightRed)
    Call SendCloseConv(index)
End If

End Sub

Public Sub AdvanceQuest(ByVal index As Long, ByVal QuestIndex As Long, ByVal TNC) 'task needed to complete

'make sure that the quest is started
If Player(index).PlayerQuest(QuestIndex).QuestStatus = 1 Then
    'make sure that the task is the task we need to complete
    If Player(index).PlayerQuest(QuestIndex).TaskOn = TNC Then
        'if so, go to the next task and check to see if we finished the quest
        Player(index).PlayerQuest(QuestIndex).TaskOn = Player(index).PlayerQuest(QuestIndex).TaskOn + 1
        Call PlayerMsg(index, "You have completed a portion of the quest " & Trim$(Quest(QuestIndex).Name) & "!", BrightRed)
        Call SendPlayerData(index)
        Call CheckFinishQuest(index, QuestIndex, Player(index).PlayerQuest(QuestIndex).TaskOn)
    End If
Else
    If Player(index).PlayerQuest(QuestIndex).QuestStatus = 0 Then Call PlayerMsg(index, "You haven't started this quest.", BrightRed)
    If Player(index).PlayerQuest(QuestIndex).QuestStatus = 2 Then Call PlayerMsg(index, "You've already finished this quest.", BrightRed)
    Call SendCloseConv(index)
End If
