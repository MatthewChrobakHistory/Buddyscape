...
Public Sub InitMessages()
    ...
    HandleDataSub(CSendConvoEvent) = GetAddress(AddressOf HandleConvoEvent)
    ...
    HandleDataSub(CSWarpTo) = GetAddress(AddressOf HandleSWarpTo)
End Sub
...
Dim sMes As String
    ...
    If GetPlayerPOINTS(index) > 0 Then
        ...
        Select Case PointType
            Case Stats.Attack
                Call SetPlayerStat(index, Stats.Attack, GetPlayerRawStat(index, Stats.Attack) + 1)
                sMes = "Attack"
            Case Stats.Strength
                Call SetPlayerStat(index, Stats.Strength, GetPlayerRawStat(index, Stats.Strength) + 1)
                sMes = "Strength"
            Case Stats.Defense
                Call SetPlayerStat(index, Stats.Defense, GetPlayerRawStat(index, Stats.Defense) + 1)
                sMes = "Defense"
            Case Stats.Agility
...
Call PlayerMsg(index, "Attack: " & GetPlayerStat(i, Attack) & " //" & "Strength: " & GetPlayerStat(i, Strength) & " // " & "Defense: " & GetPlayerStat(i, Defense) & " // Agility: " & GetPlayerStat(i, Agility), White)
...
Sub HandleWarpMeTo(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    If n <> index Then
        If n > 0 Then
            Call SimplePlayerWarp(index, GetPlayerMap(n), GetPlayerX(n), GetPlayerY(n))
                If Player(index).Sprite <> 0 Then Call PlayerMsg(n, GetPlayerName(index) & " has warped to you.", BrightBlue)
...
Sub HandleWarpToMe(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    If n <> index Then
        If n > 0 Then
            Call SimplePlayerWarp(n, GetPlayerMap(index), GetPlayerX(index), GetPlayerY(index))
            If Player(index).Sprite <> 0 Then Call PlayerMsg(n, "You have been summoned by " & GetPlayerName(index) & ".", BrightBlue)
...
' :: Warp to map packet ::
' ::::::::::::::::::::::::
Sub HandleSWarpTo(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    Dim n As Long
    Dim Buffer As clsBuffer
    Set Buffer = New clsBuffer
    Buffer.WriteBytes Data()
    ...
    Call SimplePlayerWarp(index, n, GetPlayerX(index), GetPlayerY(index))
    Call PlayerMsg(index, "You have been warped to map #" & n, BrightBlue)
    Call AddLog(GetPlayerName(index) & " warped to map #" & n & ".", ADMIN_LOG)
End Sub
...
Sub HandleMapData(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    Map(mapnum).InstanceHost = Buffer.ReadByte
    Map(mapnum).IsInstanced = Buffer.ReadByte
    Map(mapnum).MinLevel = Buffer.ReadByte
...
With Shop(shopNum).TradeItem(shopslot)
         ...
         If Shop(shopNum).ShopType > 0 Then
                 ...
                 If Item(Shop(shopNum).TradeItem(shopslot).Item).SmRew > 0 Then
                         Player(index).SmithingXP = Player(index).SmithingXP + Item(Shop(shopNum).TradeItem(shopslot).Item).SmRew * XPRate
                 End If
                 If Item(Shop(shopNum).TradeItem(shopslot).Item).CoRew > 0 Then
                         Player(index).CookingXP = Player(index).CookingXP + Item(Shop(shopNum).TradeItem(shopslot).Item).CoRew * XPRate
                 End If
                 If Item(Shop(shopNum).TradeItem(shopslot).Item).FlRew > 0 Then
                         Player(index).FletchingXP = Player(index).FletchingXP + Item(Shop(shopNum).TradeItem(shopslot).Item).FlRew * XPRate
                 End If
                 If Item(Shop(shopNum).TradeItem(shopslot).Item).CrRew > 0 Then
                         Player(index).CraftingXP = Player(index).CraftingXP + Item(Shop(shopNum).TradeItem(shopslot).Item).CrRew * XPRate
                 End If
                 If Item(Shop(shopNum).TradeItem(shopslot).Item).PBRew > 0 Then
                        Player(index).PotionBrewingXP = Player(index).PotionBrewingXP + Item(Shop(shopNum).TradeItem(shopslot).Item).PBRew * XPRate
                 End If
                 SendPlayerData (index)
         End If
...
Public Sub HandleConvoEvent(ByVal index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer
Dim PageOn As Long
Dim ConvIndex As Long
Dim Choice As Byte
Dim i As Byte
...
Set Buffer = New clsBuffer
Buffer.WriteBytes Data()
PageOn = Buffer.ReadLong
ConvIndex = Buffer.ReadLong
Choice = Buffer.ReadLong
Set Buffer = Nothing

Select Case Conv(ConvIndex).Chat(PageOn).Event
    Case 1 'bank
        Call SendBank(index)
        
    Case 2 'shop
        Call SendOpenShop(index, Conv(ConvIndex).Chat(PageOn).Data1)
        TempPlayer(index).InShop = Conv(ConvIndex).Chat(PageOn).Data1
    ...
    Case 3 'giveitem
        ...
        If FreeSlot = 0 Then
            ...
            Exit Sub
        Else
            Call GiveInvItem(index, Conv(ConvIndex).Chat(PageOn).Data1, Conv(ConvIndex).Chat(PageOn).Data2)
        End If
        
    Case 4 'takeitem
        
        FreeSlot = HasItem(index, Conv(ConvIndex).Chat(PageOn).Data1)
        If FreeSlot = 0 Or FreeSlot < Conv(ConvIndex).Chat(PageOn).Data2 Then
            Call PlayerMsg(index, "You don't have the item the NPC wants!", BrightRed)
            Call SendCloseConv(index)
            Exit Sub
        End If

        Call TakeInvItem(index, Conv(ConvIndex).Chat(PageOn).Data1, Conv(ConvIndex).Chat(PageOn).Data2)
    Case 5
        If Map(Conv(ConvIndex).Chat(PageOn).Data1).MaxX < Conv(ConvIndex).Chat(PageOn).Data2 Or Map(Conv(ConvIndex).Chat(PageOn).Data1).MaxY < Conv(ConvIndex).Chat(PageOn).Data3 Then
            Call PlayerMsg(index, "The set warp values for the warp exceed the map values. Please contact a mod about this. Sorry.", BrightRed)
            Call SendCloseConv(index)
            Exit Sub
        End If
        Call PlayerWarp(index, Conv(ConvIndex).Chat(PageOn).Data1, Conv(ConvIndex).Chat(PageOn).Data2, Conv(ConvIndex).Chat(PageOn).Data3)
    Case 6 'start quest
        Call StartQuest(index, Conv(ConvIndex).Chat(PageOn).Data1)
    Case 7 'advance quest
        'make sure that the quest admits it needs to be activated by chat
        If Quest(Conv(ConvIndex).Chat(PageOn).Data1).Task(Conv(ConvIndex).Chat(PageOn).Data2).TaskType <> 1 Then Exit Sub
        Call AdvanceQuest(index, Conv(ConvIndex).Chat(PageOn).Data1, Conv(ConvIndex).Chat(PageOn).Data2)
    Case Else
        Call PlayerMsg(index, "Tis a bug. Please report this to a mod.   ^_^ Thanks!", Cyan)
End Select
