...
Sub AddChar(ByVal index As Long, ByVal Name As String, ByVal Sex As Byte, ByVal ClassNum As Long, ByVal Sprite As Long)
    ...
    If LenB(Trim$(Player(index).Name)) = 0 Then
        ...
        For n = 1 To MAX_QUESTS
            Player(index).PlayerQuest(n).DataAmountLeft = 0
            Player(index).PlayerQuest(n).QuestStatus = 0
            Player(index).PlayerQuest(n).TaskOn = 0
        Next
...
Sub LoadClasses()
    ...
    For i = 1 To Max_Classes
        ...
        ' continue
        Class(i).Stat(Stats.Attack) = Val(GetVar(FileName, "CLASS" & i, "Attack"))
        Class(i).Stat(Stats.Strength) = Val(GetVar(FileName, "CLASS" & i, "Strength"))
        Class(i).Stat(Stats.Defense) = Val(GetVar(FileName, "CLASS" & i, "Defense"))
...
Sub SaveClasses()
    ...
    For i = 1 To Max_Classes
        ...
        Call PutVar(FileName, "CLASS" & i, "Attack", STR(Class(i).Stat(Stats.Attack)))
        Call PutVar(FileName, "CLASS" & i, "Strength", STR(Class(i).Stat(Stats.Strength)))
        Call PutVar(FileName, "CLASS" & i, "Defense", STR(Class(i).Stat(Stats.Defense)))
...
Sub SaveMap(ByVal mapnum As Long)
    ...
    Put #F, , Map(mapnum).DropItemsOnDeath
    Put #F, , Map(mapnum).InstanceHost
    Put #F, , Map(mapnum).IsInstanced
    Put #F, , Map(mapnum).MinLevel
...
Sub LoadMaps()
    ...
    For i = 1 To MAX_MAPS
        ...
        Get #F, , Map(i).MaxX
        Get #F, , Map(i).MaxY
        Get #F, , Map(i).DropItemsOnDeath
        Get #F, , Map(i).InstanceHost
        Get #F, , Map(i).IsInstanced
        Get #F, , Map(i).MinLevel
...
Function GetClassMaxVital(ByVal ClassNum As Long, ByVal Vital As Vitals) As Long
    Select Case Vital
        Case HP
            With Class(ClassNum)
                GetClassMaxVital = 100 + (.Stat(Defense) * 5) + 2
            End With
        Case MP
            With Class(ClassNum)
                GetClassMaxVital = 30 + (.Stat(Agility) * 10) + 2
            End With
    End Select
End Function
