...
Begin VB.Form frmMain 
   ...
   Begin VB.PictureBox picAdmin 
      ...
      Visible         =   0   'False
      Width           =   2865
      Begin VB.CommandButton ASWarpTo 
         Caption         =   "(s)Warp To"
         Height          =   255
         Left            =   240
         TabIndex        =   147
   ...
   Begin VB.PictureBox picOptions 
      ...
      Begin VB.OptionButton optMNOn 
         ...
         Width           =   735
      End
      Begin VB.OptionButton optMNOff 
         Appearance      =   0  'Flat
         BackColor       =   &H00000000&
         Caption         =   "Off"
         BeginProperty Font 
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    If Player(MyIndex).SecOutofCombat = 0 Then
        logoutGame
    Else
        AddText "You must wait 10 seconds out of combat before logging out.", BrightRed
    End If
...
lblMiningXP.Caption = "XP: " & Player(MyIndex).MiningXP
lblWoodcuttingXP.Caption = "XP: " & Player(MyIndex).WoodcuttingXP
lblFishingXP.Caption = "XP: " & Player(MyIndex).FishingXP
lblPotionBrewingXP.Caption = "XP: " & Player(MyIndex).PotionBrewingXP
lblSmithingXP.Caption = "XP: " & Player(MyIndex).SmithingXP
lblCookingXP.Caption = "XP: " & Player(MyIndex).CookingXP
lblFletchingXP.Caption = "XP: " & Player(MyIndex).FletchingXP
lblCraftingXP.Caption = "XP: " & Player(MyIndex).CraftingXP
...
Private Sub lblConvNPCName_Click()
...
If StatState = CHAR_STATE_RAW Then
    StatState = CHAR_STATE_EQUIP
    lblStats.Caption = "Equipment Stats"
Else
    StatState = CHAR_STATE_RAW
    lblStats.Caption = "Raw Stats"
End If

For i = 1 To Stats.Stat_Count - 1
    If StatState = CHAR_STATE_RAW Then
        frmMain.lblCharStat(i).Caption = GetPlayerRawStat(MyIndex, i)
    Else
        frmMain.lblCharStat(i).Caption = GetPlayerStat(MyIndex, i) - GetPlayerRawStat(MyIndex, i)
    End If
Next
...
Index = lstQuest.ListIndex + 1

lblQName.Caption = Trim$(Quest(Index).Name)
lblQSummary.Caption = QuestInfo(Index).Summary
lblQTask.Caption = QuestInfo(Index).Task(Player(MyIndex).PlayerQuest(Index).TaskOn).IntroInfo

Select Case Player(MyIndex).PlayerQuest(Index).QuestStatus
    Case 0
        lblQStatus.Caption = "- Not Started -"
        lblQStatus.ForeColor = &HFF&
    Case 1
        lblQStatus.Caption = "- Started -"
        lblQStatus.ForeColor = &H80FF&
    Case 2
        lblQStatus.Caption = "- Completed -"
        lblQStatus.ForeColor = &HFF00&
End Select

End Sub

Private Sub optMNOff_Click()

Options.MiniMap = False

End Sub

Private Sub optMNOn_Click()
...
For i = 1 To MAX_QUESTS
    If Player(MyIndex).PlayerQuest(i).QuestStatus = 0 Then lstQuest.AddItem (Trim$(Quest(i).Name) & "    Not Started " & Player(MyIndex).PlayerQuest(i).TaskOn)
    If Player(MyIndex).PlayerQuest(i).QuestStatus = 1 Then lstQuest.AddItem (Trim$(Quest(i).Name) & "    In Progress " & Player(MyIndex).PlayerQuest(i).TaskOn)
    If Player(MyIndex).PlayerQuest(i).QuestStatus = 2 Then lstQuest.AddItem (Trim$(Quest(i).Name) & "    Finished " & Player(MyIndex).PlayerQuest(i).TaskOn)
Next

If picBook.Visible = False Then picBook.Visible = True
...
Dim XP As Long
    ...
    Select Case Index
        Case 0
            Skill = " Mining "
            XP = Player(MyIndex).MiningXP
        Case 1
            Skill = " Fishing "
            XP = Player(MyIndex).FishingXP
        Case 2
            Skill = " Woodcutting "
            XP = Player(MyIndex).WoodcuttingXP
        Case 3
            Skill = " Herblore "
            XP = Player(MyIndex).PotionBrewingXP
        Case 4
            Skill = " Smithing "
            XP = Player(MyIndex).SmithingXP
        Case 5
            Skill = " Cooking "
            XP = Player(MyIndex).CookingXP
        Case 6
            Skill = " Fletching "
            XP = Player(MyIndex).FletchingXP
        Case 7
            Skill = " Crafting "
            XP = Player(MyIndex).CraftingXP
    End Select
    
    Call AddText("You have " & XP & Skill & " XP.", Cyan)

    Exit Sub
errorhandler:
    HandleError "picSkill_Click(" & Index & ")", "frmmain", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Exit Sub
End Sub
...
With frmMain
    .lblConvMessage.Caption = Conv(ConvIndex).Chat(PageOn).Text
    For i = 1 To 4
        
        .lblConvResp(i).Visible = True
        
        If Conv(ConvIndex).Chat(PageOn).QuestRequirement(i).Index > 0 Then
            Select Case Conv(ConvIndex).Chat(PageOn).QuestRequirement(i).Condition
                Case 1 'is equal to
                    If Player(MyIndex).PlayerQuest(Conv(ConvIndex).Chat(PageOn).QuestRequirement(i).Index).TaskOn <> Conv(ConvIndex).Chat(1).QuestRequirement(i).Task Then
                        .lblConvResp(i).Visible = False
                    End If
        ...
        .lblConvResp(i).Caption = Conv(ConvIndex).Chat(PageOn).ReplyText(i)
        If .lblConvResp(i).Caption = vbNullString Then
            .lblConvResp(i).Enabled = False
        Else
            .lblConvResp(i).Enabled = True
        End If
    Next
End With
