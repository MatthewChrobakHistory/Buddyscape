...
Dim Y As Long
    ...
    Put #f, , Map.InstanceHost
    Put #f, , Map.IsInstanced
    Put #f, , Map.MinLevel
...
Dim Y As Long
    ...
    Get #f, , Map.InstanceHost
    Get #f, , Map.IsInstanced
    Get #f, , Map.MinLevel
...
End Sub
Sub SetPlayerLNIM(ByVal Index As Long, ByVal LNIM As Long)
If Index > MAX_PLAYERS Then Exit Sub
Player(Index).LNIM = LNIM
End Sub
...
Public Function GetPlayerSpeed(ByVal Index As Long, ByVal Run As Boolean) As Byte
Dim i As Long
Dim FinalSpeed As Long

FinalSpeed = 4 + GetPlayerStat(Index, Agility) / 11

For i = 1 To Equipment.Equipment_Count - 1
    If GetPlayerEquipment(Index, i) > 0 Then
        FinalSpeed = FinalSpeed - (Item(GetPlayerEquipment(Index, i)).Weight / 10)
    End If
Next

'Call AddText(FinalSpeed, Pink)

If FinalSpeed < 2 Then FinalSpeed = 2 'min speed
If Run = True Then FinalSpeed = FinalSpeed + 2
If FinalSpeed > 8 Then FinalSpeed = 8 'max speed

'Call AddText(FinalSpeed, Cyan)
GetPlayerSpeed = FinalSpeed

End Function

Public Function GetNpcSpeed(ByVal Index As Long) As Byte
Dim FinalSpeed As Long

FinalSpeed = Npc(Index).Speed
If FinalSpeed = 0 Then FinalSpeed = 4

If FinalSpeed > 8 Then FinalSpeed = 8
If FinalSpeed < 2 Then FinalSpeed = 2

GetNpcSpeed = FinalSpeed
