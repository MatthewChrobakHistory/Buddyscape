...
Dim rec_pos As DxVBLib.RECT
    ...
    BltBars
     ...
     ' minimap
    If Options.MiniMap = True Then BltMiniMap
    
    ' Blt the target icon
    If myTarget > 0 Then
        If myTargetType = TARGET_TYPE_PLAYER Then
            If Player(myTarget).Sprite <> 0 Then
        ...
        ElseIf myTargetType = TARGET_TYPE_NPC Then
            If Npc(MapNpc(myTarget).Num).Sprite <> 0 Then
    ...
    For i = 1 To Player_HighIndex
        If IsPlaying(i) Then
            If Player(i).Map = Player(MyIndex).Map Then
                ...
                'MOD HIDING
                    If myTargetType = TARGET_TYPE_PLAYER And myTarget = i Or GetPlayerSprite(i) = 0 Then
                        ' dont render lol
                    Else
    ...
    For i = 1 To Npc_HighIndex
        If MapNpc(i).Num > 0 Then
            If CurX = MapNpc(i).x And CurY = MapNpc(i).Y Then
                'MOD HIDING
                If myTargetType = TARGET_TYPE_NPC And myTarget = i Or Npc(MapNpc(i).Num).Sprite = 0 Then
                    ' dont render lol
                Else
...
Dim BlockRect As RECT, WarpRect As RECT, ItemRect As RECT, NpcRect As RECT, PlayerRect As RECT, BlankRect As RECT, ResourceRect As RECT, MapItemRect As RECT
Dim TreeRect As RECT, RockRect As RECT, FishingRect As RECT
...
For x = 0 To Map.MaxX
    For Y = 0 To Map.MaxY
    ' Camera loc
    CameraX = Camera.Left + 25 + (x * 4) '340
    CameraY = Camera.top + 25 + (Y * 4) '25
    ...
    For i = 1 To NOMI
        If MapItem(i).Num > 0 Then
            If MapItem(i).x = x And MapItem(i).Y = Y Then
                Engine_BltFast CameraX, CameraY, DDS_MiniMap, MapItemRect, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY
            End If
        End If
    Next
    ...
    Select Case Map.Tile(x, Y).Type
        ...
        Case TILE_TYPE_RESOURCE
            If Resource(Map.Tile(x, Y).Data1).ResourceType = 0 Then Engine_BltFast CameraX, CameraY, DDS_MiniMap, ResourceRect, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY
            If Resource(Map.Tile(x, Y).Data1).ResourceType = 1 Then Engine_BltFast CameraX, CameraY, DDS_MiniMap, TreeRect, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY
            If Resource(Map.Tile(x, Y).Data1).ResourceType = 2 Then Engine_BltFast CameraX, CameraY, DDS_MiniMap, RockRect, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY
            If Resource(Map.Tile(x, Y).Data1).ResourceType = 3 Then Engine_BltFast CameraX, CameraY, DDS_MiniMap, FishingRect, DDBLTFAST_WAIT Or DDBLTFAST_DESTCOLORKEY
    End Select
    Next Y
Next x
...
For i = 1 To Player_HighIndex
    If IsPlaying(i) And Player(i).Sprite <> 0 Then
...
For i = 1 To Npc_HighIndex
    If MapNpc(i).Num <> 0 Then
        If MapNpc(i).Num > 0 And Npc(MapNpc(i).Num).Sprite <> 0 Then
            ' Npc loc
            x = MapNpc(i).x
            Y = MapNpc(i).Y
            ...
            Call Engine_BltFast(CameraX, CameraY, DDS_MiniMap, NpcRect, DDBLTFAST_WAIT Or DDBLTFAST_SRCCOLORKEY)
        End If
    End If
Next i
...
Public Function NumberofMapItems() As Long
Dim i As Long
Dim Num As Long
