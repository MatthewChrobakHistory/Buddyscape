...
Dim MovementSpeed As Long
    ...
    Select Case Player(index).Moving
        Case MOVING_WALKING: MovementSpeed = ((ElapsedTime / 1000) * (GetPlayerSpeed(MyIndex, False) * SIZE_X))
        Case MOVING_RUNNING: MovementSpeed = ((ElapsedTime / 1000) * (GetPlayerSpeed(MyIndex, True) * SIZE_X))
        Case Else: Exit Sub
    End Select
...
Sub ProcessNpcMovement(ByVal MapNpcNum As Long)
    ...
    If MapNpc(MapNpcNum).Moving = MOVING_WALKING Then
        ...
        Select Case MapNpc(MapNpcNum).Dir
            Case DIR_UP
                MapNpc(MapNpcNum).YOffset = MapNpc(MapNpcNum).YOffset - ((ElapsedTime / 1000) * (GetNpcSpeed(MapNpc(MapNpcNum).Num) * SIZE_X))
            ...
            Case DIR_LEFT
                MapNpc(MapNpcNum).XOffset = MapNpc(MapNpcNum).XOffset - ((ElapsedTime / 1000) * (GetNpcSpeed(MapNpc(MapNpcNum).Num) * SIZE_X))
...
Dim d As Long
    ...
    ' not in a conv
    If frmMain.picConv.Visible = True Then
        CanMove = False
        Exit Function
    End If
...
Dim i As Long
    ...
    If Map.Moral = MAP_MORAL_SAFE Or MAP_MORAL_DUNGEON Then Exit Function
...
Dim i As Byte
Dim EventRan As Boolean

'look for an event
If Conv(ConvIndex).Chat(PageOn).Event > 0 Then
    Call SendConvoEvent(PageOn, ConvIndex, Choice)
    EventRan = True
End If

'exit out the conv if there's nothing to continue to
If Conv(ConvIndex).Chat(PageOn).ReplyConvTo(Choice) = 0 Or EventRan = True Then
...
'if not, continue
PageOn = Conv(ConvIndex).Chat(PageOn).ReplyConvTo(Choice)
    'visibles
    With frmMain
    ...
    If Conv(ConvIndex).Chat(PageOn).SoundLength > 0 Then
        If Options.Sound = 1 Then
            For i = 1 To 4
                frmMain.lblConvResp(i).Visible = False
            Next
            frmMain.tmrSound.Interval = Conv(ConvIndex).Chat(PageOn).SoundLength
            frmMain.tmrSound.Enabled = True
        End If
    End If
    
    If Conv(ConvIndex).Chat(PageOn).Sound <> vbNullString Then PlaySound Trim$(Conv(ConvIndex).Chat(PageOn).Sound)
