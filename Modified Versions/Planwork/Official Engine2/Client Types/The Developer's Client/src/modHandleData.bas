...
Public Sub InitMessages()
    ...
    HandleDataSub(SSendStartConv) = GetAddress(AddressOf HandleStartConversation)
...
Dim Buffer As clsBuffer
    ...
    frmMenu.tmrLogin.Enabled = False
...
Dim i As Long
    ...
    For i = 1 To Stats.Stat_Count - 1
        ...
        ElseIf StatState = CHAR_STATE_EQUIP Then
...
Dim Buffer As clsBuffer
    ...
    Call SetPlayerLNIM(i, Buffer.ReadLong)
    ...
    If i = MyIndex Then
        ...
        For x = 1 To Stats.Stat_Count - 1
            If StatState = CHAR_STATE_RAW Then
                frmMain.lblCharStat(x).Caption = GetPlayerRawStat(MyIndex, x)
            Else
                frmMain.lblCharStat(x).Caption = GetPlayerStat(MyIndex, x) - GetPlayerRawStat(MyIndex, x)
            End If
        Next
...
Dim MapNum As Long
    ...
    Map.InstanceHost = Buffer.ReadByte
    Map.IsInstanced = Buffer.ReadByte
    Map.MinLevel = Buffer.ReadByte
...
Private Sub HandleStartConversation(ByVal Index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer
Dim CVIndex As Long
...
Dim i As Byte
    ...
    Set Buffer = New clsBuffer
    Buffer.WriteBytes Data()
    CVIndex = Buffer.ReadLong
    NPCIndex = Buffer.ReadLong
    Set Buffer = Nothing
    
    'set the values for the future
    ConvIndex = CVIndex
    PageOn = 1
    ...
    With frmMain
        ...
        .lblConvMessage.Caption = Conv(CVIndex).Chat(1).Text
        For i = 1 To 4
        
            If Conv(CVIndex).Chat(1).QuestRequirement(i).Index > 0 Then
                Select Case Conv(CVIndex).Chat(1).QuestRequirement(i).Condition
                    Case 1 'is equal to
                        If Player(MyIndex).PlayerQuest(Conv(CVIndex).Chat(1).QuestRequirement(i).Index).TaskOn <> Conv(CVIndex).Chat(1).QuestRequirement(i).Task Then
            ...
            .lblConvResp(i).Caption = Trim$(Conv(CVIndex).Chat(1).ReplyText(i))
        ...
        If Npc(NPCIndex).Face > 0 Then .picConvFace.Picture = LoadPicture(App.Path & "\data files\graphics\faces\" & Npc(NPCIndex).Face & ".bmp")
    End With
    
    If Conv(CVIndex).Chat(1).SoundLength > 0 Then
        If Options.Sound = 1 Then
            For i = 1 To 4
                frmMain.lblConvResp(i).Visible = False
            Next
            frmMain.tmrSound.Interval = Conv(CVIndex).Chat(1).SoundLength
            frmMain.tmrSound.Enabled = True
        End If
    End If
    
    ' play the sound
    If Conv(CVIndex).Chat(1).Sound <> vbNullString Then
        PlaySound Trim$(Conv(CVIndex).Chat(1).Sound)
    End If
    
Exit Sub
errorhandler:
    Call AddText("This NPC's face is not valid. Please tell this to a mod, and tell them this number: " & NPCIndex, BrightRed)
