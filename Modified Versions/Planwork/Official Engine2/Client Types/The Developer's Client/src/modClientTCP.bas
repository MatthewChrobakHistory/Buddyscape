...
Dim Buffer As clsBuffer
    ...
    With Map
        ...
        Buffer.WriteByte .InstanceHost
        Buffer.WriteByte .IsInstanced
        Buffer.WriteByte .MinLevel
    End With
...
Public Sub SWarpTo(ByVal MapNum As Long)
Dim Buffer As clsBuffer
...
Public Sub SendConvoEvent(ByVal PageOn As Long, ByVal ConvoIndex As Long, ByVal Choice As Byte)
Dim Buffer As clsBuffer

    If Options.Debug = 1 Then On Error GoTo errorhandler
    
    Set Buffer = New clsBuffer
    Buffer.WriteLong CSendConvoEvent
    Buffer.WriteLong PageOn
    Buffer.WriteLong ConvoIndex
    ...
    Exit Sub
errorhandler:
    HandleError "SendConvoEvent", "modClientTCP", Err.Number, Err.Description, Err.Source, Err.HelpContext
    Err.Clear
    Exit Sub
End Sub
