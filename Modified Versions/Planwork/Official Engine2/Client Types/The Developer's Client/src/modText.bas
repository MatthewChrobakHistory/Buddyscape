...
Dim Level As String
    ...
    'MOD HIDING
    If GetPlayerSprite(Index) = 0 Then Exit Sub
...
Dim Level As String
    ...
    'MOD HIDING
    If Npc(npcNum).Sprite = 0 Then Exit Sub
    ...
    Level = vbNullString
    If Npc(npcNum).Level > 0 Then Level = "Level " & Npc(npcNum).Level
    TextX = ConvertMapX(MapNpc(Index).x * PIC_X) + MapNpc(Index).XOffset + (PIC_X \ 2) - getWidth(TexthDC, (Trim$(Name)))
    Text2X = ConvertMapX(MapNpc(Index).x * PIC_X) + MapNpc(Index).XOffset + (PIC_X / 2) - getWidth(TexthDC, Level)
    ...
    Else
        ...
        Text2Y = ConvertMapY(MapNpc(Index).Y * PIC_Y) + MapNpc(Index).YOffset - (DDSD_Character(Npc(npcNum).Sprite).lHeight / 4) + 32
    End If
                ...
                If IsValidMapPoint(x, Y) Then
                    With Map.Tile(x, Y)
                        ...
                        Select Case .Type
                            ...
                            'MATT TILE
                            Case TILE_TYPE_REC
                                DrawText TexthDC, tX, tY, "R", QBColor(Cyan)
                        End Select
                    End With
                End If
            Next
